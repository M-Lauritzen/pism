...
class SSAForwardRunFromInputFile(SSAForwardRun):
    ...
    def __init__(self, input_filename, inv_data_filename, design_var, grid_variables=None):
        ...
        grid_variables : list of str, optional
            Variable names used with ``Grid.FromFile`` to determine the grid
            dimensions from *input_filename* (e.g. ``["thk"]`` for a 2-D file
            that has no ``enthalpy`` variable). When ``None`` (default),
            ``Grid.FromOptions`` is used and the grid must be specified via
            command-line flags.
        """
    ...
    def _initGrid(self):
        ...
        if self._grid_variables is not None:
            ...
            self.grid = PISM.Grid.FromFile(ctx, input_file, self._grid_variables, PISM.CELL_CENTER)
        else:
    ...
    def _initPhysics(self):
        ...
        # No else: preserve any flow law already set in config before setup() was called.
    ...
    def _initCoefficients(self):
        ...
        for v in [thickness, bed]:
            v.regrid(self.input_filename, True)
        if PISM.util.fileHasVariable(self.input_filename, "enthalpy"):
            enthalpy.regrid(self.input_filename, True)
        else:
            enthalpy.set(80000.0)  # constant ~-10 °C; irrelevant for isothermal_glen flow law
